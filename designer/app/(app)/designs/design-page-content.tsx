...
const DesignPageContent = ({ initialDesigns }: { initialDesigns: Design[] }) => {
    const { data: designs, isPending, isError, error, fetchStatus, status: queryStatus} = useQuery({
        queryKey: ["designs"],
        queryFn: async () => {
    ...
    useEffect(() => {
        console.log('Query state:', { isFetching: isPending, isError, error, designsLength: designs?.length, fetchStatus, queryStatus })
        ...
        return (
            designs?.filter((design) => {
                const matchesQuery = query
                    ...
                    : true;
                const matchesStatus =
                    status && status !== 'all' ? design.status === status.toUpperCase() : true;
                const matchesCategory =
                    category && category !== 'all' ? design.designCategoryId === category : true;
