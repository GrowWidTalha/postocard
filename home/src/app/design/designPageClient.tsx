...
import { useCartStore } from "@/hooks/useCart"; // our zustand cart hook with persistence
...
const DesignPageClient = ({ design }: { design: Design }) => {
  ...
  // This effect re-draws the canvas whenever the page or text changes.
  useEffect(() => {
  ...
  }, [pageIndex, inputText, design, canvasData, setCanvasData]);
  ...
  const getPageContent = (index: number) => {
    switch (index) {
      ...
      case 1:
        return (
          <img
            src={design.pdfLink || "/placeholder.svg"}
            alt="Design PDF"
            className="w-full h-full object-contain"
          />
        );
      case 2:
        return (
          <canvas
            ...
            width={300}
            height={420}
            className="w-full h-full border-4 border-dotted border-pink-200"
          />
        );
      case 3:
        return (
